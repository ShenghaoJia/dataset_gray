...
int test_camera()
{
    ...
    /* shooting param */
    	...
    	char n[10] = {0};
	char postfix[10] = {0};
    // 在这里修改图片保存的位置，例如 ../../../dataset/bazhang/img_gray_80_ 会保存在根目录的dataset/bazhang文件夹下，并且以 img_gray_80_+编号为名字命名图片。
	strcpy(prefix, "../../../dataset/bazhang/img_gray_80_");
	strcpy(postfix, ".ppm");
        //itoa(number, n, 10);
        sprintf(n,"%d",number);
        strcat(prefix, n);
...
// Write to file
        ...
        WriteImageToFile(prefix, WIDTH, HEIGHT, sizeof(uint8_t), buff_demosaick, GRAY_SCALE_IO);
        printf("Finished!\n");
#endif
